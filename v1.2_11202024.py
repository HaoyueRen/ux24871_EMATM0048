...
def MaximumSupply(LabourInDays2,resources2,Index):
    ...
    for y in resources2.keys():
        maximumValues.append(int(resources2[x]/fishes[Index][y]))
    ...
    for i in demands.keys():
        ...
        while True:
            ...
            if LabourInDays < float(fishes[i]['Maintenance Time (in days)']*quantity):
                ...
                MaximumSupply(LabourInDays,resources0,i)
                continue
            #check fertilizer
            if resources0['Fertilizer(litres)']< float(fishes[i]['Fertilizer(litres)']*quantity):
                print("Insufficient fertilizers: required {0} litres, available {1} litres. Please choose again.".format(fishes[i]['Fertilizer(litres)']*quantity, resources0['Fertilizer(litres)']))
                MaximumSupply(LabourInDays,resources0,i)
                continue
            #check feed
